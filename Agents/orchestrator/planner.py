...
class Planner:
    ...
    def _heuristic_candidates(question: str) -> List[AgentName]:
        ...
        # Use dynamic query pattern detection from capability registry
        pattern_matches = _REGISTRY.detect_query_patterns(question)
        
        # Calculate base scores from keywords
        sql_score = sum(keyword in lowered for keyword in _SQL_KEYWORDS) * 2  # Double weight for SQL keywords
        ...
        # Apply dynamic pattern boosts from capability registry
        for agent_name, match_info in pattern_matches.items():
            boost = match_info["total_boost"]
            if agent_name == "sql":
                sql_score += boost
                logger.info(
                    f"SQL pattern match detected: boost={boost}, "
                    f"patterns={[m['pattern'] for m in match_info['matches']]}"
                )
            elif agent_name == "api_docs":
                api_score += boost
            elif agent_name == "computation":
                comp_score += boost
        ...
        if detected_metrics:
            for metric in detected_metrics:
                ...
                if api_tools:
                    ...
                    logger.info(f"Metric '{metric}' available via API, but SQL bias will prefer sql agent")
                    break
        
        # BIAS: Prefer SQL agent over API agent (with increased weight)
        # SQL provides more flexibility and direct database access
        if sql_score > 0:
            # SQL gets priority if it has any score
            order.append("sql")
        elif has_api_metrics:
            # Only use API if SQL keywords not present
            order.append("api_docs")
        elif api_score >= max(comp_score, graph_score) and api_score > 0:
            order.append("api_docs")
        ...
        if not order:
            # BIAS: Default to SQL (more flexible than API)
            order = ["sql"]
    ...
    def _plan_from_decomposition(
        ...
        *,
        ...
        for sub_query in sorted_sub_queries:
            ...
            agents = [a for a in agents if a not in disable]
